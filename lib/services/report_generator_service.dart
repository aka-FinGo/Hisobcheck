...
class ReportGeneratorService {
  ...
  static Future<void> generateExcel(List<Map<String, dynamic>> data, String title, List<String> columns) async {
    try {
      ...
      final path = "${dir.path}/$title_${DateTime.now().millisecondsSinceEpoch}.xlsx";
  ...
  static Future<void> generatePdf(List<Map<String, dynamic>> data, String title, List<String> columns) async {
    try {
      ...
      final dir = await getTemporaryDirectory();
      final path = "${dir.path}/$title_${DateTime.now().millisecondsSinceEpoch}.pdf";
      final file = File(path);
  ...
  // 3. BONUS: RASMLI INFOGRAFIKA GENERATORI (Canvas orqali yozildi)
  static Future<void> generateImageInfographic(Map<String, String> summaryStats, String title) async {
    try {
      ...
      // Statistikani bloklarga bo'lib chizish
      double currentY = 200;
      ...
      // Rasmni xotiraga olish (Flutter rasmni nativ PNG sifatida saqlaydi)
      ...
      final dir = await getTemporaryDirectory();
      // PNG sifatida saqlanib ulashiladi (Sifatli va JPG bilan bir xil ishlaydi)
